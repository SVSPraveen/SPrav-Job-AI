...
def init_data_dir():
    ...
    # Only migrate files that are safe to overwrite and don't contain user state.
    # User state like config.json, scope.json, jobs.db, etc. should NEVER be in this list.
    migration_files = [
        # Put any non-mutable assets here if needed in the future
    ]
...
def load_kb(path: str = None) -> dict:
    """Helper to safely load candidate knowledge base from JSON."""
    import json
    if path is None:
        path = os.path.join(get_data_dir(), "knowledge_base", "me.json")
